...
public:
        ...
        , v(new QDoubleValidator(parent))
    {
        ...
        parent->lineEdit()->setValidator(v.data());
    }

    // create q-pointer for parent class, USE Q_Q for access
    QScienceSpinBox * q_ptr;
...
// otherwise QScopedPointer wont be deleted
...
bool QScienceSpinBoxPrivate::isIntermediateValue(const QString &str) const
{
    Q_Q(const QScienceSpinBox);
    QSBDEBUG() << "input is" << str << q->minimum() << q->maximum();
    qint64 dec = 1;

    for (int i=0; i < q->decimals(); ++i)
        dec *= 10;
    ...
    */
    ...
    const QString minstr = QString::number(q->minimum(), 'f',  static_cast<QDoubleSpinBox*>(q_ptr)->decimals());
    qint64 min_left = minstr.left(minstr.indexOf(dot)).toLongLong();
    qint64 min_right = minstr.mid(minstr.indexOf(dot) + 1).toLongLong();

    const QString maxstr = QString::number(q->maximum(), 'f', static_cast<QDoubleSpinBox*>(q_ptr)->decimals());
    ...
    const bool negative = q->maximum() < 0;
...
/*!
    ...
    reimplemented function, copied from QDoubleSpinBoxPrivate::validateAndInterpret
*/
QVariant QScienceSpinBoxPrivate::validateAndInterpret(
    QString &input,
    int &pos,
    QValidator::State &state) const
{
    ...
    else if (len > 1)
    {
        ...
        if (dec != -1) {
            ...
            if (copy.size() - dec > static_cast<QDoubleSpinBox*>(q_ptr)->decimals() + 1) {
                QSBDEBUG() << __FILE__ << __LINE__<< "state is set to Invalid";
                state = QValidator::Invalid;
                goto end;
            }
    ...
    cachedText = q->prefix() + copy + q->suffix();
    cachedState = state;
...
/*!
    ...
    reimplemented function, copied from QAbstractSpinBoxPrivate::stripped
*/
QString QScienceSpinBoxPrivate::stripped(const QString &t, int *pos) const
{
    Q_Q(const QScienceSpinBox);
    QString text = t;
    QString prefixtext = q->prefix();
    QString suffixtext = q->suffix();
