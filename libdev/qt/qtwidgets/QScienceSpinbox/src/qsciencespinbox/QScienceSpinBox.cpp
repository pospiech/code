...
{
public:
    QScienceSpinBoxPrivate(QScienceSpinBox * parent)
...
private:
    QScienceSpinBox * parentSpinBox;

public:
    int dispDecimals;
    QChar delimiterChr, thousandChr;
    QScopedPointer<QDoubleValidator> v;

    void initLocalValues(QWidget *parent)
    {
        const QString str = (parent ? parent->locale() : QLocale()).toString(4567.1);
        if (str.size() == 6) {
            delimiterChr = str.at(4);
            thousandChr = QChar((ushort)0);
        } else if (str.size() == 7) {
            thousandChr = str.at(1);
            delimiterChr = str.at(5);
        }
        Q_ASSERT(!delimiterChr.isNull());
    }
    ...
    {
        QSBDEBUG() << "input is" << str << parentSpinBox->minimum() << parentSpinBox->maximum();
        qint64 dec = 1;

        for (int i=0; i < parentSpinBox->decimals(); ++i)
            dec *= 10;
        ...
        */
        ...
        const QString minstr = QString::number(parentSpinBox->minimum(), 'f',  static_cast<QDoubleSpinBox*>(parentSpinBox)->decimals());
        qint64 min_left = minstr.left(minstr.indexOf(dot)).toLongLong();
        qint64 min_right = minstr.mid(minstr.indexOf(dot) + 1).toLongLong();

        const QString maxstr = QString::number(parentSpinBox->maximum(), 'f', static_cast<QDoubleSpinBox*>(parentSpinBox)->decimals());
         ...
         */
        const int dotindex = str.indexOf(this->delimiterChr);
        const bool negative = parentSpinBox->maximum() < 0;
    ...
    */
        ...
        QValidator::State &state) const
    {
        ...
        const double max = parentSpinBox->maximum();
        const double min = parentSpinBox->minimum();
        ...
        // First char must not be thousandChr
        if (copy.at(0) == thousandChr)
        {
        ...
        {
            const int dec = copy.indexOf(delimiterChr); // position of delimiter
            // if decimal separator (delimiter) exists
            if (dec != -1) {
                // not two delimiters after one other (meaning something like ',,')
                if (dec + 1 < copy.size() && copy.at(dec + 1) == delimiterChr && pos == dec + 1) {
                    copy.remove(dec + 1, 1); // typing a delimiter when you are on the delimiter
                }							 // should be treated as typing right arrow
                // too many decimal points
                if (copy.size() - dec > static_cast<QDoubleSpinBox*>(parentSpinBox)->decimals() + 1) {
                    QSBDEBUG() << __FILE__ << __LINE__<< "state is set to Invalid";
                    state = QValidator::Invalid;
                    goto end;
                }
                // after decimal separator no thousandChr char
                for (int i=dec + 1; i<copy.size(); ++i) {
                    if (copy.at(i).isSpace() || copy.at(i) == thousandChr) {
                        QSBDEBUG() << __FILE__ << __LINE__<< "state is set to Invalid";
                        state = QValidator::Invalid;
                        goto end;
                    }
                ...
                // group of two thousandChr or space chars is invalid
                if ((last == thousandChr || last.isSpace())
                        && (secondLast == thousandChr || secondLast.isSpace())) {
                    state = QValidator::Invalid;
                    QSBDEBUG() << __FILE__ << __LINE__<< "state is set to Invalid";
                    goto end;
                }
                // two space chars is invalid
                else if (last.isSpace() && (!thousandChr.isSpace() || secondLast.isSpace())) {
                    state = QValidator::Invalid;
                    QSBDEBUG() << __FILE__ << __LINE__<< "state is set to Invalid";
                    goto end;
                }
        ...
        // block of remaining test before 'end' mark
        {
            ...
            QLocale loc(parentSpinBox->locale());
            ...
            if (!ok) {
                // maybe thousandChr char was responsable
                if (thousandChr.isPrint())
                {
                    // if no thousandChr sign is possible, then
                    // something else is responable -> Invalid
                    if (max < 1000 && min > -1000 && copy.contains(thousandChr)) {
                        state = QValidator::Invalid;
                        QSBDEBUG() << __FILE__ << __LINE__<< "state is set to Invalid";
                        goto end;
                    }

                    // two thousandChr-chars after one other are not valid
                    const int len = copy.size();
                    for (int i=0; i<len- 1; ++i) {
                        if (copy.at(i) == thousandChr && copy.at(i + 1) == thousandChr) {
                            QSBDEBUG() << __FILE__ << __LINE__<< "state is set to Invalid";
                            state = QValidator::Invalid;
                            goto end;
                        }
                    ...
                    // remove thousandChr-chars
                    const int s = copy.size();
                    copy.remove(thousandChr);
                    pos = qMax(0, pos - (s - copy.size()));

                    num = loc.toDouble(copy, &ok);
                    QSBDEBUG() << thousandChr << num << copy << ok;
                ...
                } // endif: (thousandChr.isPrint())
            }

            // no thousandChr sign, but still invalid for unknown reason
            if (!ok) {
                state = QValidator::Invalid;
                QSBDEBUG() << __FILE__ << __LINE__<< "state is set to Invalid";
            }
        ...
        cachedText = parentSpinBox->prefix() + copy + parentSpinBox->suffix();
        cachedState = state;
    ...
    */
    ...
    {
        QString text = t;
        QString prefixtext = parentSpinBox->prefix();
        QString suffixtext = parentSpinBox->suffix();

        if (parentSpinBox->specialValueText().size() == 0 || text != parentSpinBox->specialValueText()) {
            int from = 0;
            int size = text.size();
            bool changed = false;
...
QScienceSpinBox::QScienceSpinBox(QWidget * parent)
    : QDoubleSpinBox(parent),
      d(new QScienceSpinBoxPrivate(this))
      //d_ptr(new QScienceSpinBoxPrivate(this))
{
    // Q_D(QScienceSpinBox)
    d->initLocalValues(parent);
    setDecimals(8);
    QDoubleSpinBox::setDecimals(1000);
    // set validator, must be here because lineEdit is protected.
    lineEdit()->setValidator(d->v.data());
...
QString QScienceSpinBox::textFromValue(double value) const
{
    ...
    // remove thousandChr sign
    if (qAbs(value) >= 1000.0) {
        str.remove(d->thousandChr);
    }
    return str;
}
